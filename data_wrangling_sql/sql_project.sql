...
Massage Room 1, Massage Room 2, Squash Court
...
SELECT CONCAT( m.surname,  ", ", m.firstname ) AS name, f.name AS facility, 
...
SELECT *
FROM(SELECT CONCAT( m.surname,  ", ", m.firstname ) AS name, f.name AS facility, 
	CASE WHEN m.surname =  'GUEST'
		THEN b.slots * f.guestcost
		ELSE b.slots * f.membercost
		END AS cost
	...
	INNER JOIN Bookings AS b ON m.memid = b.memid
	INNER JOIN Facilities AS f ON b.facid = f.facid
	WHERE starttime BETWEEN "2012-09-14" AND "2012-09-15") as temp
WHERE cost > 30
ORDER BY cost DESC
...
FROM (SELECT f.name AS facility, SUM( 
	CASE WHEN b.memid =0
